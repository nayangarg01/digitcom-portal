...
# CONFIG
# ──────────────────────────────────────────────
BASE_DICT_FILE = "../Routing_Logic_V2/km_required_2_enriched.xlsx"
MANUAL_FILE    = "../Routing_Logic_V2/KM A6 MMWAVE.xlsx"
...
# MAIN PIPELINE
# ──────────────────────────────────────────────
def build_jc_cmp_map():
    df_base = pd.read_excel(BASE_DICT_FILE).fillna("")
    jc_map = {}
    for _, row in df_base.iterrows():
        jc = str(row.get('JC NAME', '')).strip().upper()
        cmp = str(row.get('CMP', '')).strip().upper()
        if jc and cmp and jc not in jc_map:
            jc_map[jc] = cmp
            
    # Add manual fallbacks if spelling is different
    jc_map['KISHANGARH'] = 'AJMER'
    jc_map['HANUMANGARH'] = 'GANGANAGAR'
    return jc_map

def run_direct_validation(api_key):
    print("Initializing mapping dictionary...")
    jc_cmp_map = build_jc_cmp_map()
    ...
    for idx, row in df_manual.iterrows():
        ...
        band = str(row.get('Activity', 'UNKNOWN')).strip().upper()
        
        # Inject CMP via Map
        jc = str(row.get('JC', '')).strip().upper()
        cmp_val = jc_cmp_map.get(jc, jc) 
    ...
    for (band, date_val, cmp_name), group_df in groups:
        ...
        for r_idx, route in enumerate(routes):
            ...
            for s_idx, leg in enumerate(route):
                ...
                # Assign True Base Route Distance prior to subtraction (as per visual validation)
